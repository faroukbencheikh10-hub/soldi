...
export interface Levels {
  ultime2h: LevelWindow | null;
  ultime6h: LevelWindow | null;
  ultime8h: LevelWindow | null;
...
function finestra(candles: RawCandle[], quante: number, prezzo: number): LevelWindow | null {
...
function contaMomentum(candles: RawCandle[]): Momentum | null {
...
export function computeLevels(
  candles15m: RawCandle[] | undefined,
  ...
  if (!Array.isArray(candles15m) || candles15m.length < 9) return vuoto;
  if (!Number.isFinite(prezzoAttuale)) return vuoto;

  const precedenti = candles15m.slice(1, 9);
  ...
  const inAtr = (v: number | null) =>
    v !== null && atr15m !== null && atr15m > 0 ? Number((v / atr15m).toFixed(2)) : null;

  return {
    ultime2h: finestra(candles15m, 8, prezzoAttuale),
    ultime6h: finestra(candles15m, 24, prezzoAttuale),
    ultime8h: finestra(candles15m, 32, prezzoAttuale),
    rotturaRialzo,
    rotturaRibasso,
    rotturaRialzoInAtr: inAtr(rotturaRialzo),
    rotturaRibassoInAtr: inAtr(rotturaRibasso),
    momentum: contaMomentum(candles15m),
  };
}
