...
export async function runAnalysis(options?: { force?: boolean }) {
  ...
  // DIREZIONE — ICT originale (Huddleston).
  ...
  const strutturaM15Pre = marketSnapshot.ictStrutturaM15 as
    | { bias?: string; evento?: string | null; direzioneEvento?: string | null }
    | undefined;
  const setupIct = valutaSetupIctOriginale({
    prezzo: Number(marketSnapshot.xauusd),
    atr15m: marketSnapshot.atr15m ?? null,
    strutturaM15: strutturaM15Pre,
    biasH1: biasH1Pre,
    biasH4: biasH4Pre,
    killZone: marketSnapshot.killZone,
    judas: marketSnapshot.judasSwing,
    displacement15m: marketSnapshot.rigetto15m,
    orderBlocksM15: marketSnapshot.ictOrderBlocksM15,
    fvgM15: marketSnapshot.ictFvgM15,
    oteM15: marketSnapshot.oteM15,
  });
  const direzioneCodice = setupIct.ok ? setupIct.direzione : null;
  const fonteDirezione = setupIct.ok ? "M15-evento" : null;
  const notaHtF = setupIct.ok
    ? `ICT originale Huddleston. ${setupIct.motivo} Zona: ${setupIct.zona}. Ripeti ${setupIct.direzione} oppure NO_TRADE.`
    : setupIct.motivo;
  const tradeProposto =
    setupIct.ok && setupIct.direzione && setupIct.entry != null
      ? {
          direzione: setupIct.direzione,
          entry: setupIct.entry,
          stop_loss: setupIct.stopLoss as number,
          tp1: setupIct.tp1 as number,
          tp2: setupIct.tp2 as number,
          rischio_rendimento: setupIct.rischioRendimento,
          nota: notaHtF,
        }
      : null;

  if (!force && !setupIct.ok) {
    const motivo = setupIct.motivo;
    const skippedSignal = validateSignal({
      direction: "NO_TRADE",
      entry: null,
      stopLoss: null,
  ...
  if ((signal.direction === "BUY" || signal.direction === "SELL") && tradeProposto) {
    ...
    signal = validateSignal({
      ...
      tp2: 0,
      riskReward: 0,
      reasoning: `${signal.reasoning ?? ""}\n\n[Nessun trade: il codice non ha potuto costruire direzione e livelli (nessun verso su H1/H4/M15, o ATR15m mancante).]`,
    } as never);
  }
