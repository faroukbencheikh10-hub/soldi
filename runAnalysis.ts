...
// ESECUZIONE ICT (04/09, sera).
// Entry, stop e target li decide l'AI seguendo il metodo: entry sul bordo
// della zona di pullback (OB/FVG, fascia OTE), stop oltre l'estremo dello
// sweep, target sulla prossima liquidita'. Il codice valida solo che i numeri
// siano coerenti (SL < entry < TP1, rapporto 1.5-4, vedi validateSignal) e
// che il trade sia ESEGUIBILE ORA: se il prezzo ha gia' lasciato la zona
// senza tornarci, non si emette nessun segnale e non si aspetta il ritorno.
//
// Tolleranza sull'entry: il prezzo puo' trovarsi appena dal lato sfavorevole
// dell'entry (entro questa frazione di ATR15m) e il trade e' ancora
// considerato eseguibile a mercato. Oltre, e' un ordine pendente e va
// scartato.
const TOLLERANZA_ENTRY_ATR = 0.15;
...
// ormai superato.
//
// DOVE E' USATA OGGI: dal canale principale, come filtro "eseguibile ora"
// sul segnale appena generato dall'AI (vedi runAnalysis), e dal canale 5m
// (disattivato).
// ---------------------------------------------------------------------------
...
async function avvisaNotizia(
  ...
  // La chiave "gia' avvisato" si scrive SOLO dopo che la push e' partita.
  // Prima veniva scritta subito: se l'invio falliva (per esempio un
  // ambiente senza chiavi VAPID che condivide lo stesso database), la
  // notizia risultava avvisata senza che nessuno avesse ricevuto niente, e
  // i cicli successivi restavano zitti. Ora un invio fallito lascia la
  // chiave com'e' e il ciclo dopo riprova, finche' la finestra e' aperta.
  // sendPushToAll non lancia se non c'e' nessuna iscrizione o se tutti gli
  // invii falliscono: restituisce i conteggi. Si controlla quindi anche
  // "sent", non solo l'assenza di eccezioni.
  try {
    const esito = await sendPushToAll({
    ...
    if (!esito || esito.sent === 0) {
      console.error("[runAnalysis] avviso notizia: nessuna push consegnata, si riprova al prossimo ciclo");
      return false;
    }
  } catch (err) {
    console.error("[runAnalysis] avviso notizia fallito (si riprova al prossimo ciclo):", err);
    return false;
  }
...
// Controllo notizia indipendente dal resto del ciclo: scarica il calendario e
// manda l'avviso se un dato e' imminente. Gira anche con trade aperto, con
// segnale in attesa, in pausa AI e quando il setup e' invariato -- un avviso
// di calendario non dipende dal grafico. Restituisce il calendario cosi' il
// chiamante puo' riusarlo senza scaricarlo due volte.
async function controllaNotizia(
  snapshot: MarketSnapshot,
  adesso: number
): Promise<Awaited<ReturnType<typeof getEconomicCalendar>>> {
  const calendar = await getEconomicCalendar().catch(() => []);
  try {
    await avvisaNotizia(calendar, snapshot, adesso);
  } catch (err) {
    console.error("[runAnalysis] controllo notizia fallito:", err);
  }
  return calendar;
}
...
export async function runAnalysis(options?: { force?: boolean }) {
    ...
    if (naturalOutcome) {
      ...
      if (expired) {
        ...
        try {
          ...
          // Anche con un trade aperto l'avviso di notizia deve partire: e'
          // proprio il momento in cui un dato macro conta di piu'.
          await controllaNotizia(freshSnapshot, Date.now());
        } catch (err) {
  ...
  // 5bis) AVVISO NOTIZIA -- qui, nel monitor, e non dopo i filtri.
  // Prima stava dopo "setup invariato" e dopo la pausa AI: bastava che il
  // grafico non fosse cambiato dal ciclo precedente perche' il ciclo uscisse
  // prima di guardare il calendario, e l'avviso non partiva mai. Il
  // calendario scaricato qui viene riusato piu' sotto per l'AI.
  const calendar = await controllaNotizia(marketSnapshot, adesso);
  ...
  const news = await getRelevantNews().catch(() => []);
  ...
  // ESEGUIBILE ORA, altrimenti niente (04/09, sera).
  //
  // I livelli sono dell'AI e sono gia' passati da validateSignal. Qui resta
  // un solo controllo: il prezzo deve essere gia' sull'entry (o oltre dal
  // lato favorevole, o entro TOLLERANZA_ENTRY_ATR dal lato sfavorevole).
  // Se il prezzo ha lasciato la zona di pullback senza tornarci, il setup
  // non e' eseguibile: NO_TRADE, nessuna attesa del ritorno.
  if (signal.direction === "BUY" || signal.direction === "SELL") {
    ...
    const entroTolleranza =
      ingresso.stato === "in_attesa" &&
      ingresso.distanzaInAtr !== null &&
      ingresso.distanzaInAtr <= TOLLERANZA_ENTRY_ATR;
    const eseguibile = ingresso.stato === "eseguibile" || entroTolleranza;
    ...
    if (!eseguibile) {
      signal = validateSignal({
        ...
        riskReward: 0,
        reasoning:
          `${signal.reasoning ?? ""}\n\n[Nessun trade: setup ${signal.direction} valido ma non eseguibile ora. ` +
          `${ingresso.testo}. Non si aspetta il ritorno sull'entry.]`,
      } as never);
    } else {
      signal = validateSignal({
        ...(signal as unknown as Record<string, unknown>),
        reasoning:
          `${signal.reasoning ?? ""}\n\n[Esecuzione ICT: livelli decisi dall'AI, ` +
          `entry ${Number(signal.entry).toFixed(2)} eseguibile al prezzo ${Number(marketSnapshot.xauusd).toFixed(2)}` +
          `${entroTolleranza ? ` (entro ${TOLLERANZA_ENTRY_ATR} ATR dall'entry)` : ""}.]`,
      } as never);
    }
  }
  ...
  if (signal.direction === "BUY" || signal.direction === "SELL") {
    ...
    //
    // Il segnale arriva qui solo se e' eseguibile ora (controllo sopra):
    // nasce gia' attivo (attivato_il valorizzato da insertSignal) e la
    // notifica parte subito.
